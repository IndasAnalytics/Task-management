...
import { Task, User, TaskPriority, JobType, WorkflowStage } from "../types";
...
export const analyzeArtworkRisk = async (fileName: string) => {
    // In a real app, we would send the image bytes to Gemini 2.5 Flash.
    // Here we simulate the structured response based on file name keywords.
...
export const getDashboardInsights = async (tasks: Task[]) => {
    ...
    // Simple cleaning of the response to get lines
